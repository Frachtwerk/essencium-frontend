...
Essencium consists of a core library (`lib`), a translations package (`translations`) and a boilerplate (`app`) that consums those packages. Additonally, we have a `docs` package to document the project.
...
Translations and i18next are managed in `translations` package in the corresponding language files.
