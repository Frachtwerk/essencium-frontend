...
For enforcing code style and utilizing static code analyzing we use [ESLint v9](https://eslint.org/) with Flat Config along with a bunch of rulesets and [Prettier](https://prettier.io/). The most
...
Import and export rules are now handled by the `simple-import-sort` plugin which provides automatic sorting without the complexity of the legacy import plugin rules.
