...
Currently, the release process is pretty minimalistic. The developer must bump the `version` field inside the root `package.json` file and commit the change. Then, a Github Action workflow is triggered and synces all versions of each `packages.json` file in the packages to the version stated inside the root `package.json` and releases the packages on npm.
